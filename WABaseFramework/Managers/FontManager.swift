...
class FontManager: BaseManager {
    ...
    public class func style(forKey key: String) -> Float {
        ...
        minFontSize = CGFloat.init(FontManager.sharedInstance.style(forKey: "minimumFontSize"))
        
        var resizedFontSize = DesignUtility.getValueFromRatio(CGFloat(FontManager.sharedInstance.style(forKey: key)))
        
        if resizedFontSize < minFontSize {
            resizedFontSize = minFontSize;
        }
        
        return Float(resizedFontSize)
